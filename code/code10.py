...
def construct_prompt(d):
    ...
    example_assistant_content = (
        ...
        "现在进入推荐的重排序阶段。请对剩余的10部候选电影进行逐一评估，结合之前分析得到的用户偏好特点，深入比较每部电影与用户喜好的契合程度（考虑题材、主题、导演、演员等因素）。然后根据每部电影的匹配度高低进行排序，像推荐系统的精排过程一样将匹配度最高的电影排在前面。请输出按匹配度从高到低排序的电影推荐 JSON 列表。"
        "[{\"name\": \"Aladdin\", \"id\": 444}]\n"
    )
