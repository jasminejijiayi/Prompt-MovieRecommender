...
import requests
import numpy as np
from code2 import construct_prompt, parse_output
import logging
import sys
import os
...
# 配置日志，保存到 result/test.log
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler(os.path.join('result', 'test2.log'), encoding='utf-8'),
...
def call_llm_api(messages, model="deepseek/deepseek-chat-v3-0324:free", temperature=0):
    ...
    for i, msg in enumerate(messages):
        if msg['role'] == 'system':
            ...
            if i == 1:  # 第一轮用户消息（分析请求）
                print(f"\n[用户第一轮 - 分析请求]:\n  {msg['content'][:10000]}..." if len(msg['content']) > 10000 else f"\n[用户第一轮 - 分析请求]:\n  {msg['content']}")
            elif i > 1:  # 第二轮用户消息（输出结果请求）
                print(f"\n[用户第二轮 - 结果请求]:\n  {msg['content']}")
...
def calculate_ndcg_for_sample(predicted_list, ground_truth_item, k=10):
    ...
    print(f"目标ID {ground_truth_id} 在预测列表中{'出现' if ground_truth_id in predicted_list else '未出现'}")
    if ground_truth_id in predicted_list:
        position = predicted_list.index(ground_truth_id) + 1
        print(f"目标ID出现在第{position}位置")
    
    # 计算DCG (Discounted Cumulative Gain)
    dcg = 0
    dcg_breakdown = []
    for i, rel in enumerate(relevance):
        # 使用位置 i+1 因为索引从0开始但位置从1开始
        position_discount = np.log2(i + 2)  # log2(i+2) 因为位置是从1开始，所以用i+2
        contribution = rel / position_discount if position_discount > 0 else 0
        dcg += contribution
        
        if rel > 0:
            dcg_breakdown.append(f"位置{i+1}: {rel}/{position_discount:.4f} = {contribution:.4f}")
    
    print(f"DCG计算: {' + '.join(dcg_breakdown) if dcg_breakdown else '0'} = {dcg:.4f}")
    ...
    for i, rel in enumerate(ideal_relevance):
        ...
        if rel > 0:
            idcg_breakdown.append(f"位置{i+1}: {rel}/{position_discount:.4f} = {contribution:.4f}")
    
    print(f"IDCG计算: {' + '.join(idcg_breakdown) if idcg_breakdown else '0'} = {idcg:.4f}")
    
    # 如果IDCG为0（即没有相关项），则NDCG定义为0
    if idcg == 0:
        print("IDCG为0，NDCG定义为0")
        return 0
    
    # 计算NDCG
    ndcg = dcg / idcg
    print(f"NDCG计算: {dcg:.4f} / {idcg:.4f} = {ndcg:.4f}")
    return ndcg
...
def test_on_sample(sample_data):
    ...
    # 使用code2.py的函数构造提示词（思维链引导策略）
    print("\n==== 构造提示词（思维链引导策略）====")
    print("调用构造多轮对话的construct_prompt函数...")
    all_messages = construct_prompt(sample_data)
    print(f"生成了{len(all_messages)}条消息，实现思维链引导的提示策略")
    
    # 说明思维链引导策略
    print("\n思维链引导策略说明：")
    print("1. 第一轮：引导模型先分析用户历史偏好特征和候选电影的属性匹配度")
    print("2. 第二轮：要求模型给出最终排序结果，使用JSON数组格式输出")
    
    # === 分别执行思维链的两轮对话 ===
    
    # 列出初始消息列表
    system_msg = all_messages[0]  # 系统消息
    first_user_msg = all_messages[1]  # 第一轮用户消息（分析请求）
    second_user_msg = all_messages[2]  # 第二轮用户消息（结果请求）
    
    # 第一轮：模型分析用户偏好和候选电影匹配度
    print("\n==== 第一轮调用API（分析用户偏好）====")
    first_round_messages = [system_msg, first_user_msg]
    print(f"发送消息: 系统提示 + 第一轮用户消息（分析请求）")
    
    analysis_response = call_llm_api(first_round_messages)
    if not analysis_response:
        print("\n==== 错误 ====")
        print("第一轮 API调用失败，无法获取分析回应")
        return 0
    ...
    second_round_messages = [
        system_msg, 
        first_user_msg,
        {"role": "assistant", "content": analysis_response},  # 添加模型的分析回应
        second_user_msg
    ]
    print(f"发送消息: 系统提示 + 第一轮用户消息 + 模型分析回应 + 第二轮用户消息（结果请求）")
    
    final_response = call_llm_api(second_round_messages)
    if not final_response:
        print("\n==== 错误 ====")
        print("第二轮 API调用失败，无法获取排序结果")
        return 0
