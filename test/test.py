...
import requests
import numpy as np
from code1 import construct_prompt, parse_output
import logging
import sys
import os
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join('result', 'test.log'), encoding='utf-8'),
...
def call_llm_api(messages, model="deepseek/deepseek-chat-v3-0324:free", temperature=0):
    ...
    for i, msg in enumerate(messages):
        print(f"消息 {i+1}:")
        print(f"  角色: {msg['role']}")
        print(f"  内容: {msg['content'][:10000]}..." if len(msg['content']) > 10000 else f"  内容: {msg['content']}")
...
def test_on_sample(sample_data):
    ...
    # 使用template.py的函数构造提示词
    print("\n==== 构造提示词 ====")
    print("调用construct_prompt函数...")
    messages = construct_prompt(sample_data)
