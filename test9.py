...
import requests
import numpy as np
from code9 import construct_prompt, parse_output
import logging
import sys
import os
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join('result', 'test9.log'), encoding='utf-8'),
...
def test_on_sample(sample_data):
    ...
    # 使用code9.py的函数构造提示词（包含Few-shot示例）
    print("\n==== 构造提示词 ====")
    print("调用construct_prompt函数(包含少样本示例提示策略)...")
    messages_template = construct_prompt(sample_data)
    print(f"生成了{len(messages_template)}条消息模板，其中包含Few-shot示例对话")
    
    # 执行第一轮对话：分析用户偏好
    print("\n==== 第一轮调用API(用户分析)结果 ====\n")
    # 第一轮发送system消息、few-shot示例和第一个user消息
    first_round_messages = [
        messages_template[0],  # system消息
        messages_template[1],  # few-shot示例用户消息
        messages_template[2],  # few-shot示例助手响应
        messages_template[3]   # 第一个用户消息(分析用户偏好)
    ]
    print(f"发送了 {len(first_round_messages)} 条消息进行用户分析（system + few-shot示例 + user第一轮消息）")
    ...
    second_round_messages = [
        ...
        messages_template[5]   # 第二个user消息(分析候选电影)
    ]
    print(f"发送了 {len(second_round_messages)} 条消息进行候选电影分析（包含few-shot示例和前两轮历史）")
    ...
    final_round_messages = [
        ...
        messages_template[3],  # 第一个用户消息(分析用户偏好)
        {"role": "assistant", "content": first_response},  # 第一轮助手回复
        messages_template[5],  # 第二个用户消息(分析候选电影)
        {"role": "assistant", "content": second_response},  # 第二轮助手回复
        messages_template[7]   # 第三个user消息(要求输出JSON)
    ]
    print("==== 发送的消息 (最终推荐请求) ====")
    print(f"[用户第三轮 - 输出要求]:\n  {final_round_messages[7]['content']}")
